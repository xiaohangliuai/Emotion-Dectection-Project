...
import joblib
from track_utils import create_page_visited_table, add_page_visited_details, view_all_page_visited_details, add_prediction_details, view_all_prediction_details, create_emotionclf_table, NYT  # Import NYT from track_utils
...
def main():
    st.title("Emotion Detection In Textual Data")
    ...
    if choice == "Home":
        add_page_visited_details("Home", datetime.now(NYT))
        ...
        if submit_text:
            ...
            add_prediction_details(raw_text, prediction, np.max(probability), datetime.now(NYT))
        ...
        add_page_visited_details("Monitor", datetime.now(NYT))
        ...
        add_page_visited_details("About", datetime.now(NYT))
